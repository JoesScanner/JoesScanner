        ...
        public string SelectedFilterProfileNameOption
        {
            ...
            set
            {
                ...
                else if (string.Equals(newValue, CustomProfileNameOption, StringComparison.Ordinal))
                {
                    ...
                    // Keep the existing draft so the user can type a new name.
                }
                else
                {
                    _isCustomFilterProfileName = false;
...
if (!string.IsNullOrWhiteSpace(draft) && _filterProfileNameOptions.Any(n => string.Equals(n, draft, StringComparison.OrdinalIgnoreCase)))
            {
        ...
        {
            var profiles = await _filterProfileStore.GetProfilesAsync(FilterProfileContexts.Archive, CancellationToken.None);
...
var selectedId = Preferences.Get(SelectedFilterProfileIdPreferenceKey, string.Empty);
        ...
        public async Task<FilterProfile?> SaveCurrentFiltersAsync(string name)
        {
            ...
            var profile = new FilterProfile
            {
                ...
                Name = name,
                Context = FilterProfileContexts.Archive,
                Filters = new FilterProfileFilters
                {
                    ...
                    SelectedDateLocal = SelectedDate.Date,
        ...
        public async Task<bool> RenameSelectedProfileAsync(string newName)
        {
            ...
            await _filterProfileStore.RenameAsync(FilterProfileContexts.Archive, current.Id, newName, CancellationToken.None);
            await LoadFilterProfilesAsync(applySelectedProfile: false);
        ...
        public async Task<bool> DeleteSelectedProfileAsync()
        {
            ...
            await _filterProfileStore.DeleteAsync(FilterProfileContexts.Archive, current.Id, CancellationToken.None);
        ...
        private void ApplyProfileToFilters(FilterProfile profile)
        {
            ...
            if (f.SelectedDateLocal != null)
                SelectedDate = f.SelectedDateLocal.Value;
